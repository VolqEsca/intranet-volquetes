...
// Función manejo fechas Excel especializada
function processExcelDate($value) {
    ...
    if (is_string($value)) {
        ...
        // Formato del Excel: "YYYY-MM-DD HH:MM:SS"
    ...
    // Si es numérico, formato Excel serial
    if (is_numeric($value)) {
        try {
...
function calculateProportionalAnnualDays($hire_date, $target_year) {
    ...
    if ($hire_year > $target_year) return 0;
    if ($hire_year < $target_year) return 22;
...
try {
    ...
    // Preparar statements reutilizables fuera del bucle (eficiencia)
    $stmtCheck = $pdo->prepare("SELECT id FROM employees WHERE dni_nie = ?");

    $stmtUpdate = $pdo->prepare("
        UPDATE employees
        SET location=?, employee_code=?, full_name=?, social_security_number=?,
            hire_date=?, rgpd_date=CURRENT_DATE, phone=?, email_primary=?,
            email_secondary=?, birth_date=?, job_category=?, gender=?,
            contract_type=?, contract_end_date=?, iban=?, updated_at=NOW()
        WHERE dni_nie=?
    ");
    ...
    for ($i = 1; $i < count($data); $i++) {
        ...
        try {
            // Ignorar filas completamente vacías (columnas 0-12)
            $is_row_empty = true;
            ...
            // Email corporativo por defecto si viene vacío (columna email NOT NULL)
            ...
            // Sexo no existe en este Excel — queda en NULL
            $gender = null;
            ...
            // Verificar si ya existe por DNI/NIE
            $stmtCheck->execute([$dni_nie]);
            $exists = (bool) $stmtCheck->fetch();

            if ($exists) {
                // UPDATE — no tocamos vacation_balances para preservar históricos
                $stmtUpdate->execute([
                    $location, $employee_code, $full_name, $social_security_number,
                    $hire_date, $phone, $email_primary, $email_secondary,
                    $birth_date, $job_category, $gender, $contract_type,
                    $contract_end_date, $iban_clean,
                    $dni_nie
                ]);
                $updated++;
            } else {
                // INSERT — inicializar saldo de vacaciones solo para nuevos
                $stmtInsert->execute([
                    $location, $employee_code, $full_name, $dni_nie, $social_security_number,
                    $hire_date, $phone, $email_primary, $email_secondary,
                    $birth_date, $job_category, $gender, $contract_type,
                ...
                $imported++;
                $new_employee_id = (int) $pdo->lastInsertId();

                // =========================================================
                // VERSO v14.1: Auto-inicialización Saldos Vacaciones
                // Solo para INSERTs, nunca en UPDATEs
                // =========================================================
                try {
                    $annual_days = calculateProportionalAnnualDays($hire_date, $current_year);
                    $stmtBalance->execute([$new_employee_id, $current_year, $annual_days]);
                } catch (Exception $e) {
                    // Silenciar — no detener importación por error de saldo
                }
                // =========================================================
            }
    ...
    error_log("Error PDO employees/import.php: " . $e->getMessage());
    http_response_code(500);
